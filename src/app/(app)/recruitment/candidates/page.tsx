    ...
    <div className="space-y-5">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl dark:bg-dark-2">
            <div className="flex items-center justify-between border-b border-gray-3 px-6 py-4 dark:border-dark-3">
              <h2 className="text-base font-bold text-dark dark:text-white">Add Candidate</h2>
              <button
            ...
            <div className="space-y-4 px-6 py-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Full Name *</label>
                  <input
                    type="text"
                    value={form.fullName}
                    onChange={(e) => setForm((f) => ({ ...f, fullName: e.target.value }))}
                    className="h-9 w-full rounded-lg border border-gray-3 bg-white px-3 text-sm outline-none focus:border-primary-600 dark:border-dark-3 dark:bg-dark-3 dark:text-white"
                    placeholder="e.g. Rahul Gupta"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Email *</label>
                  <input
                    type="email"
                    value={form.email}
                ...
                <div>
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Phone</label>
                  <input
                    type="tel"
                    value={form.phone}
                ...
                <div>
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Role Applying For</label>
                  <select
                    value={form.role}
                ...
                <div>
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Source</label>
                  <select
                    value={form.source}
                ...
                <div className="col-span-2">
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Resume Link</label>
                  <input
                    type="text"
                    value={form.resumeLink}
                    onChange={(e) => setForm((f) => ({ ...f, resumeLink: e.target.value }))}
                    className="h-9 w-full rounded-lg border border-gray-3 bg-white px-3 text-sm outline-none focus:border-primary-600 dark:border-dark-3 dark:bg-dark-3 dark:text-white"
                    placeholder="Paste LinkedIn or drive link"
                  />
                </div>
                <div className="col-span-2">
                  <label className="mb-1 block text-xs font-semibold text-dark dark:text-white">Notes</label>
                  <textarea
                    rows={2}
                    value={form.notes}
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-3 px-6 py-4 dark:border-dark-3">
              <button
                onClick={() => setShowAddModal(false)}
                className="rounded-lg border border-gray-3 px-4 py-2 text-sm font-medium text-dark hover:bg-gray-2 dark:border-dark-3 dark:text-white dark:hover:bg-dark-3"
              >
                Cancel
              </button>
              <button
