...
export default function OffboardingPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setShowModal(false);
    setForm({ employeeName: "", employeeId: "", lastWorkingDay: "", reason: "", exitInterviewDate: "", hrContact: "" });
          ...
          return (
            ...
            >
              ...
              <div className="flex gap-2 border-t border-gray-3 pt-3 dark:border-dark-3">
                <button
                  onClick={() => setToast("Opening offboarding details...")}
                  className="flex-1 rounded-lg border border-gray-3 py-2 text-xs font-medium text-dark-5 hover:bg-gray-2 dark:border-dark-3 dark:text-dark-6"
                >
                  View Details
                </button>
                <button
                  onClick={() => setToast("Reminder sent to employee!")}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-modal dark:bg-dark-2">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-dark dark:text-white">Initiate Offboarding</h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-5 dark:text-dark-6 mb-1">Employee ID</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-5 dark:text-dark-6 mb-1">Reason</label>
                  <select
                    value={form.reason}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-dark-5 dark:text-dark-6 mb-1">Exit Interview Date</label>
                  <input
                    type="date"
                    value={form.exitInterviewDate}
                    onChange={(e) => setForm((p) => ({ ...p, exitInterviewDate: e.target.value }))}
                    className="w-full rounded-lg border border-gray-3 bg-white px-3 py-2 text-sm text-dark outline-none focus:border-primary-600 dark:border-dark-3 dark:bg-dark-3 dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-dark-5 dark:text-dark-6 mb-1">HR Contact</label>
                  <select
                    value={form.hrContact}
                    onChange={(e) => setForm((p) => ({ ...p, hrContact: e.target.value }))}
                    className="w-full rounded-lg border border-gray-3 bg-white px-3 py-2 text-sm text-dark outline-none focus:border-primary-600 dark:border-dark-3 dark:bg-dark-3 dark:text-white"
                  >
