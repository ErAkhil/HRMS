  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-dark dark:text-white">Workflows</h1>
          <p className="mt-0.5 text-xs text-dark-5 dark:text-dark-6">Automate HR processes and notifications</p>
        </div>
        <button
      ...
      {workflows.length === 0 ? (
        <div className="rounded-xl bg-white p-10 shadow-card dark:bg-dark-2 text-center">
          <p className="text-dark-5 dark:text-dark-6 mb-3">No workflows yet.</p>
          <button
            onClick={() => setShowModal(true)}
        ...
        <div className="space-y-3">
          {workflows.map((wf) => (
            <div key={wf.id} className="rounded-xl bg-white p-5 shadow-card dark:bg-dark-2 dark:border dark:border-dark-3">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-semibold text-dark dark:text-white">{wf.name}</h3>
                  ...
                  {wf.description && (
                    <p className="mt-1 text-sm text-dark-5 dark:text-dark-6">{wf.description}</p>
                  )}
                  <div className="mt-2 flex flex-wrap gap-3 text-xs text-dark-5 dark:text-dark-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-modal dark:bg-dark-2">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-dark-5 dark:text-dark-6 mb-1">Name <span className="text-rose-500">*</span></label>
                <input
                  required
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium text-dark-5 dark:text-dark-6 mb-1">Trigger <span className="text-rose-500">*</span></label>
                <input
                  required
                  type="text"
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="rounded-lg border border-gray-3 px-4 py-2 text-sm font-medium text-dark hover:bg-gray-2 dark:border-dark-3 dark:text-white">
                  Cancel
                </button>
