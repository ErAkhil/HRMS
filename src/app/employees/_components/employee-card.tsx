    ...
    <div className="rounded-xl bg-white p-5 shadow-card dark:bg-dark-2 dark:border dark:border-dark-3 flex flex-col">
      ...
      <div className="flex items-start justify-between">
        <div className="relative">
          ...
          />
          <span
            className={`absolute bottom-0.5 right-0.5 h-3 w-3 rounded-full border-2 border-white dark:border-dark-2 ${STATUS_COLORS[employee.status]}`}
            title={employee.status}
          />
        </div>
        <span className={DEPARTMENT_COLORS[employee.department]}>
          {employee.department}
        </span>
      </div>
