        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {MY_CERTIFICATIONS.map((cert) => (
            ...
            >
              ...
              <button className="w-full rounded-lg border border-gray-3 py-1.5 text-xs font-medium text-dark-5 hover:bg-gray-2 dark:border-dark-3 dark:text-dark-6">
          ...
          {AVAILABLE_CERTIFICATIONS.map((cert) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${cert.fee === "Free" ? "bg-emerald-light text-emerald-dark dark:bg-emerald-dark/20 dark:text-emerald" : "bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-300"}`}>
                  {cert.fee}
                </span>
              </div>
              <p className="text-sm font-semibold text-dark dark:text-white mb-1">{cert.name}</p>
              ...
              <button className="w-full rounded-lg bg-primary-600 py-1.5 text-sm font-semibold text-white hover:bg-primary-700">
                Register
              </button>
            </div>
          ))}
