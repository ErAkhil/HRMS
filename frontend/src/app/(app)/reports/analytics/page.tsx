...
export default async function AdvancedAnalyticsPage() {
  const { hireAttrition, deptStats } = await getAnalyticsData().catch(() => ({
    hireAttrition: [] as { month: string; hires: number; attrition: number }[],
    deptStats: [] as { dept: string; headcount: number; openRoles: number }[],
  }));
  ...
  return (
    <div className="space-y-5">
        ...
        {deptStats.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {deptStats.map((row) => {
                  const vacancyRate = row.headcount > 0
                    ? ((row.openRoles / (row.headcount + row.openRoles)) * 100).toFixed(0)
                    : "0";
                  return (
                    <tr key={row.dept} className="border-b border-gray-3 last:border-0 dark:border-dark-3 hover:bg-gray-1 dark:hover:bg-dark-3">
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`font-semibold ${row.openRoles > 0 ? "text-amber-500" : "text-emerald-500"}`}>
                          {row.openRoles}
                        </span>
                      </td>
      ...
      {/* Diversity — not yet configured */}
      <div className="rounded-xl bg-white p-5 shadow-card dark:bg-dark-2 dark:border dark:border-dark-3">
        <h2 className="text-sm font-semibold text-dark dark:text-white mb-3">Diversity Metrics</h2>
        <div className="rounded-lg bg-gray-1 dark:bg-dark-3 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-dark dark:text-white mb-1">Diversity data not yet configured</p>
              <p className="text-xs text-dark-5 dark:text-dark-6">
                Gender, ethnicity, and other diversity dimensions require optional employee profile fields to be enabled.
                Contact your HR administrator to configure diversity data collection.
              </p>
            </div>
          </div>
        </div>
