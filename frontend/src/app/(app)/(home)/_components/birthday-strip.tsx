...
function CelebrationCard({
  ...
  isWished,
  onWish,
}: Readonly<{ person: CelebrationItem; isWished: boolean; onWish: (id: string, name: string) => void }>) {
  return (
    <div className="flex items-center gap-2.5 rounded-xl border border-gray-3 px-3 py-2.5 dark:border-dark-3">
      <Image src={person.avatarUrl ?? "/images/user/user-03.png"} width={36} height={36} alt={person.name} className="rounded-full object-cover" />
      <div>
      ...
      {isWished ? (
        <button disabled className="ml-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-semibold text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-300 cursor-default">Wished ✓</button>
      ) : (
        <button
          onClick={() => onWish(person.id, person.name)}
  ...
  return (
    <div className="card-p">
      <div className="flex items-center justify-between">
        <h3 className="section-title">Celebrations this week</h3>
        <span className="rounded-full bg-amber-light px-2 py-0.5 text-[10px] font-bold text-amber-dark dark:bg-amber-dark/20 dark:text-amber">{celebrations.length} upcoming</span>
      </div>
      <div className="mt-4 flex flex-wrap gap-3">
        {celebrations.map((person) => (
          <CelebrationCard key={person.id} person={person} isWished={wished.has(person.id)} onWish={handleWish} />
        ))}
      </div>
      <Toast message={toast} />
    </div>
