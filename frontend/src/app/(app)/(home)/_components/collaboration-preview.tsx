...
function MeetingPill({ meeting }: Readonly<{ meeting: SerializedMeeting }>) {
  return (
    <div className="flex items-center gap-1.5 rounded-lg border border-emerald-light bg-emerald-light px-2.5 py-1.5 dark:border-emerald-dark/30 dark:bg-emerald-dark/10">
      <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-emerald" />
      <span className="text-[11px] font-semibold text-emerald-dark dark:text-emerald">{meeting.title}</span>
      <span className="text-[10px] text-dark-5 dark:text-dark-6">{meeting.participants.length} live</span>
    </div>
  );
}

function ChannelItem({ channel }: Readonly<{ channel: Channel }>) {
  return (
    <li>
      <Link href="/collaboration/messages" className="flex items-center gap-3 py-2.5 hover:opacity-80">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-dark dark:text-white">{channel.name}</span>
            <span className="text-[10px] text-dark-5 dark:text-dark-6">{relativeTime(channel.lastMessageAt)}</span>
          </div>
          <p className="mt-0.5 truncate text-[11px] text-dark-5 dark:text-dark-6">{channel.lastMessage ?? "No messages yet"}</p>
        </div>
    ...
    <div className="flex h-full flex-col card-p">
      ...
      <ul className="mt-3 flex-1 divide-y divide-gray-3 overflow-y-auto dark:divide-dark-3">
        {displayChannels.length === 0 ? (
          ...
          displayChannels.map((ch) => <ChannelItem key={ch.id} channel={ch} />)
        )}
      </ul>
      <MessageInputBox />
    </div>
